...
game()
{
  ...
  while [[ FRAME_NO -lt 10 ]]; do
   ...
   if [[ "$FRAME_NO" -eq 9 ]]; then
      #IF STIKE OCCURS
    if [[ "$ROLL1" -eq 10 ]]; then
      echo "Allow extra roll"
          while [[ "$ROLL3" -eq "" ]]; do
          read ROLL3?"Roll 3 Score:"
          ROLL3=`echo "$ROLL3" | grep "^[0-9][0]\?$`

          #CHECK INPUT FOR ROLL3 (ENTERED NUMBER IS VALID OR NOT)
          if [[ "$ROLL3" -eq "" ]]; then
          echo "!!Invalid input!! please try again..."
          sleep 1
          fi
          done
          SPECIAL_CASE_STRIKE=1
     fi
   fi
    ...
    elif [[ "$SPECIAL_CASE_STRIKE" -eq 1 ]]; then
      echo "END OF FRAME SCORE::: ${ENDOF_FRAME_SCORE[$(($FRAME_NO-1))]}"
      echo "ROLL1 ::: $ROLL1"
      echo "ROLL2 ::: $ROLL2"
      echo "ROLL3 ::: $ROLL3"
